...
async function main() {
  ...
  const oracle = await ethers.getContractAt("DynamicPricingOracle", deployedAddresses.oracle);
  const marketplace = await ethers.getContractAt("TicketMarketplace", deployedAddresses.marketplace);
  ...
  // const listTx = await marketplace.connect(seller).addProduct(ticketId, listingPrice);
  // await listTx.wait();
  console.log("Ticket listed successfully");

  // Request dynamic price
  console.log("\nRequesting dynamic price...");
  // const requestTx = await marketplace.connect(buyer).requestDynamicPrice(ticketId);
